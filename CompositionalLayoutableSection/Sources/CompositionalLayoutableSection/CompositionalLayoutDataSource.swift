...
open class CompositionalLayoutDataSource: NSObject, UICollectionViewDataSource {
    ...
    public func collectionView(
        ...
        if indexPath.item == 9223372036854775807, let configuration = configuration {
            return configuration.collectionView(
                collectionView,
                viewForSupplementaryElementOfKind: kind,
                at: indexPath
            )
        } else {
            return dataSource(at: indexPath)?.collectionView?(
                collectionView,
                viewForSupplementaryElementOfKind: kind,
                at: indexPath
            ) ?? UICollectionReusableView()
    ...
    public func collectionView(_ collectionView: UICollectionView, canMoveItemAt indexPath: IndexPath) -> Bool {
        dataSource(at: indexPath)?.collectionView?(
            collectionView,
    ...
    public func collectionView(
        ...
        dataSource(at: sourceIndexPath)?.collectionView?(
            collectionView,
